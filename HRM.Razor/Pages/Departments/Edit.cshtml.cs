...
namespace HRM.Razor.Pages.Departments
{
    ...
    public class EditModel : PageModel
    {
        ...
        [BindProperty]
        public EditDepartmentViewModel Input { get; set; } = new();
        ...
        public async Task<IActionResult> OnGetAsync(int id)
        {
            ...
            {
                TempData["ErrorMessage"] = "Không tìm thấy phòng ban yêu cầu.";
                return RedirectToPage("/Departments/Index");
            }

            var dept = response.Data;
            Input = new EditDepartmentViewModel
            {
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            ...
            var response = await _departmentApiClient.UpdateDepartmentAsync(id, Input);
            ...
            if (!response.Success)
            {
                ModelState.AddModelError(string.Empty, response.Message ?? "Cập nhật phòng ban thất bại.");
                await LoadActiveManagersAsync();
                return Page();
            }

            TempData["SuccessMessage"] = $"Đã cập nhật thông tin phòng ban {Input.DepartmentName}!";
            return RedirectToPage("/Departments/Index");
        }
